...
from sixpack.models import ABExperiment
...
class TestAlternativeChoice(unittest.TestCase):
    ...
    def test_bots_get_winner_otherwise_control(self):
        e = Experiment.find_or_create("bots-get-winner", "ab", ["one", "two"], redis=self.app.redis)
        # control at first
        for i in range(3):
    ...
    def test_force_param_always_wins(self):
        alts = ["one", "two", "three"]
        e = Experiment.find_or_create("force-param-always-wins", "ab", alts, redis=self.app.redis)
